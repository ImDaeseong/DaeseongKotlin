...
class DbHelperAlarm(context: Context) : SQLiteOpenHelper( context, DATABASE_NAME,null, DATABASE_VERSION) {
    ...
    fun deleteMaxData() {
        ...
        try {
            ...
            val db = this.writableDatabase
            val query = "DELETE from alarms where id = (select min(id) from alarms);"
    ...
    fun getMaxData(): Alarm {
        ...
        try {
            ...
            val query = "select * from alarms where id = (select min(id) from alarms)"
            val cursor = db.rawQuery(query, null)
            if (cursor.moveToFirst()) {
                cursor.moveToFirst()
                alarm.id = cursor.getString(0).toInt()
    ...
    fun getAlarmList(): ArrayList<Alarm> {
        ...
        try {
            ...
            val db = this.readableDatabase
            val cursor = db.rawQuery("Select id, title, content, writeDate from $TABLE_ALARM", null)
