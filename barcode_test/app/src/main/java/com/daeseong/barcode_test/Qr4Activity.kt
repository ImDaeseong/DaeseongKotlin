...
class Qr4Activity : AppCompatActivity(), ZXingScannerView.ResultHandler {
    ...
    private fun displayCode2(sCode: String) {
        ...
        try {
            val bitmap: Bitmap = encodeBitmap(sCode, BarcodeFormat.CODE_128, 300, 100)!!
    ...
    private fun encodeBitmap(sCode: String, format: BarcodeFormat, width: Int, height: Int): Bitmap? {
        val WHITE = -0x1
        val BLACK = -0x1000000
        ...
        result = try {
            writer.encode(contentsToEncode, format, width, height, hints)
    ...
    private fun encodeBitmap2(sCode: String): Bitmap {
        ...
        throw IllegalArgumentException("Could not encode barcode")
    }
